As we mentionned earlier, images unlike texts have properties which lead traditional algorithm of encryption (mostly block-cypher algorithm like AES-128, RC6...) to some drawbacks. When the algorithm is applied to a pixel at the position $(x,y)$, the values of the four bytes (RGBa) change. These modifications need to appear randomly to the attackers in order to consider the algorithm as a good encryption algorithm. We can analyze the algorithm in several ways, we will present three of them here and we will apply these methods to the proposed algorithm. One property of the dynamical systems with chaotic behaviours is the sensibility to the initial conditions, we will study how this property can be nice to avoid brute-force attacks. \\
